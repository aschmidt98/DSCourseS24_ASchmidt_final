...
# Normalize the crime rates using standard deviation (z-score normalization)
data_frame$Offense_Rate_Z <- (data_frame$Offense_Rate - mean(data_frame$Offense_Rate, na.rm = TRUE)) / sd(data_frame$Offense_Rate, na.rm = TRUE)
data_frame$Violent_Crime_Rate_Z <- (data_frame$Violent_Crime_Rate - mean(data_frame$Violent_Crime_Rate, na.rm = TRUE)) / sd(data_frame$Violent_Crime_Rate, na.rm = TRUE)
data_frame$Property_Crime_Rate_Z <- (data_frame$Property_Crime_Rate - mean(data_frame$Property_Crime_Rate, na.rm = TRUE)) / sd(data_frame$Property_Crime_Rate, na.rm = TRUE)

# Ensure all necessary columns are converted to numeric and normalized
data_frame$Population <- as.numeric(gsub(",", "", data_frame$Population))

# Remove rows with NA values in normalized Offense_Rate
data_frame <- na.omit(data_frame)
...
# Plot with a regression line using ggplot2
library(ggplot2)
plot <- ggplot(data_frame, aes(x = Population, y = Offense_Rate)) +
  geom_point(alpha = 0.6, color = "blue") +  # Plot the points with some transparency
  geom_smooth(method = "lm", color = "red", se = TRUE) +  # Add a linear regression line with standard error
  labs(title = "Relationship Between Population and Crime Rate",
       x = "Population",
       y = "Offense Rate") +
  theme_minimal() +
...
ggsave("CrimeRate_vs_Population.pdf", plot, width = 10, height = 8, units = "in")
# Or save as PNG for web use or if color is a concern
# ggsave("CrimeRate_vs_Population.png", plot, width = 10, height = 8, units = "in")
